...
function SchedulePage() {
  ...
  // Agrupar partidos por fecha y sede
  const groupedGames = sortedGames.reduce((groups, game) => {
    ...
    const venueName = getVenueName(game.venue_id) || "Sin sede";
        ...
        <div className="relative flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-foreground">
              {showOnlyFuture ? "Solo próximos" : "Todos"}
            </span>
            <Switch
              checked={showOnlyFuture}
              onCheckedChange={setShowOnlyFuture}
              aria-label="Mostrar solo partidos futuros"
            />
          </div>
        </div>
      </div>
      ...
      {gameGroups.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-primary/20 bg-gradient-to-br from-muted/30 to-background p-12 text-center">
          ...
          <p className="mt-4 text-lg font-bold text-foreground">
            {showOnlyFuture ? "No hay partidos próximos" : "No hay partidos programados"}
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            {showOnlyFuture
              ? "No hay partidos futuros en esta temporada"
        ...
        <div className="flex flex-col gap-8">
          {gameGroups.map((group) => (
            <div key={`${group.date}||${group.venue}`} className="flex flex-col gap-4">
                  ...
                  return (
                    ...
                    >
                      <Card className="schedule-card group/card h-full overflow-hidden border-2 transition-all hover:border-primary/50 hover:shadow-xl hover:-translate-y-1">
                        <div className={`relative z-10 flex items-stretch ${showScore ? "bg-gradient-to-br from-muted/40 via-background to-muted/20" : "bg-gradient-to-br from-primary/10 via-primary/5 to-background"}`}>
